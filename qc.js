...
const QC = {
      ...
      return `
        <div class="session-card" onclick="QC.openSession('${s.id}')">
          <div class="session-card-header">
            <div>
              <div class="session-id">${s.pi} · ${s.id}</div>
              <div class="session-title">${s.model}-${s.version}</div>
              <div class="session-customer">${s.customer} · ${s.color}</div>
            </div>
            ${reviewBadge}
          </div>
          <div class="session-meta">
            <span class="session-meta-item">📅 ${InspectionCore.Utils.formatDate(s.inspectionDate, lang)}</span>
            <span class="session-meta-item">📦 ${s.boxCount} ${lang === 'zh' ? '箱' : 'boxes'}</span>
            <span class="session-meta-item">🏷️ ${s.type}</span>
            ${defectCount > 0 ? `<span class="session-meta-item defect-count">⚠ ${defectCount} ${lang === 'zh' ? '缺陷' : 'defects'}</span>` : ''}
          </div>
        </div>
      `;
    }).join('');
  ...
  async openSession(sessionId) {
    ...
    document.getElementById('reviewTitle').textContent = `${session.model}-${session.version} · ${session.color}`;
    document.getElementById('reviewMeta').textContent = `${session.customer} · PI: ${session.pi} · ${session.type}`;
  ...
  renderInfo() {
    ...
    const items = [
      { label: t('form.inspector'), value: s.inspector || '—' },
      { label: t('form.inspection_date'), value: InspectionCore.Utils.formatDate(s.inspectionDate, lang) },
      { label: t('form.supplier'), value: s.supplier || '—' },
      { label: t('form.box_count'), value: s.boxCount },
      { label: t('form.sample_qty'), value: s.sampleQty || '—' },
      { label: t('form.order_qty'), value: s.orderQty || '—' },
    ];
      ...
      return `
        <div class="defect-card">
          ...
          <div class="defect-card-footer">
            <span>${lang === 'zh' ? '影响' : 'Affects'}: ${d.affected} · ${lang === 'zh' ? '第' : 'Box'} ${d.boxNumber} ${lang === 'zh' ? '箱' : ''}</span>
          </div>
        </div>
      `;
    }).join('');
      ...
      if (item.sub) {
        ...
        return `
          <div class="box-item-card">
            <div class="box-item-header">
              <div class="box-item-title">${t(item.i18n)}</div>
              <div class="box-item-status">${photos.length} ${I18n.current === 'zh' ? '张' : 'photos'}</div>
            </div>
  ...
  async approve() {
    if (!confirm(I18n.current === 'zh' ? '确认批准此检验？' : 'Approve this inspection?')) return;
  ...
  async reject() {
    ...
    if (!comment) {
      InspectionCore.Utils.toast(I18n.current === 'zh' ? '请填写拒绝原因' : 'Please add a comment', 'error');
      return;
    }
    if (!confirm(I18n.current === 'zh' ? '确认拒绝此检验？' : 'Reject this inspection?')) return;
